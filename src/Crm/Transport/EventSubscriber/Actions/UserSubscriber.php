...
final class UserSubscriber extends AbstractActionsSubscriber
{
    ...
    public function onActions(PageActionsEvent $event): void
    {
        ...
        if ($this->isGranted('view', $user)) {
            $event->addAction('profile-stats', [
                'icon' => 'avatar',
                'url' => $this->path('user_profile', [
        ...
        $subEvent = new PageActionsEvent($user, [
            'user' => $user,
        ], 'user_forms', 'index');
        ...
        if (($event->getUser()->getId() === $user->getId() && $this->isGranted('report:user')) || $this->isGranted('report:other')) {
            $event->addActionToSubmenu('report', 'weekly', [
                'url' => $this->path('report_user_week', [
                    'user' => $user->getId(),
                ]),
                'translation_domain' => 'reporting',
                'title' => 'report_user_week',
            ]);
            $event->addActionToSubmenu('report', 'monthly', [
                'url' => $this->path('report_user_month', [
                    'user' => $user->getId(),
                ]),
                'translation_domain' => 'reporting',
                'title' => 'report_user_month',
            ]);
            $event->addActionToSubmenu('report', 'yearly', [
                'url' => $this->path('report_user_year', [
                    'user' => $user->getId(),
                ]),
                'translation_domain' => 'reporting',
                'title' => 'report_user_year',
            ]);
        }
        ...
        if ($this->isGranted('view_team')) {
            $event->addActionToSubmenu('filter', 'teams', [
                'url' => $this->path('admin_team', [
                    'users[]' => $user->getId(),
                ]),
                'title' => 'teams',
            ]);
        ...
        if ($event->isIndexView() && $this->isGranted('delete', $user)) {
            $event->addDelete($this->path('admin_user_delete', [
                'id' => $user->getId(),
            ]));
        }
    }
