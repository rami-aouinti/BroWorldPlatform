            ...
            <v-row>
                <v-col lg="8" cols="12">
                    ...
                    <v-progress-circular
                        ...
                        color="primary"
                    ></v-progress-circular>
                    <v-col v-for="(post, postIndex) in this.blog" :key="post[0].id">
                        <v-card>
                            <div class="border-bottom d-flex align-center px-4 py-4">
                                <div class="d-flex align-center">
                                    ...
                                    <div class="mx-4">
                                        <a
                                            href="javascript:"
                                            class="text-h5 text-typo font-weight-bold mt-0 mb-2">
                                        ...
                                        <div class="d-flex">
                                            <div class="d-flex align-center">
                                                <v-icon
                                                    ...
                                                    :class="{
                                                        'text-primary': post['post'].isLikedByUser,
                                                        'text-body': !post['post'].isLikedByUser
                                                    }"
                                                    size="14"
                                                    class="material-icons-round me-1 cursor-pointer"
                                                >thumb_up</v-icon
                                                >
                                                <span class="text-sm text-body me-4">{{ post['post'].likesCount }}</span>
                                            </div>
                                            <div class="d-flex align-center">
                                                <v-icon
                                                    size="14"
                        ...
                        "
                                ...
                                <div class="mb-1">
                                    ...
                                    <v-col
                                        v-for="(comment, commentIndex) in post[0].comments" :key="commentIndex"
                                        class="d-flex"
                                    >
                                        <div class="d-flex">
                                        ...
                                        <div class="flex-grow-1 ms-4">
                                            <h5 class="text-dark font-weight-600 text-sm text-decoration-none">
                                                {{ comment.author.username }}
                                            </h5>
                                            <p class="text-sm font-weight-light text-body">
                                                {{ isExpanded[postIndex] && isExpanded[postIndex][commentIndex] ? comment.content : truncatedText(comment.content) }}
                                                <span v-if="!isExpanded[postIndex] && comment.content.length > 100">...</span>
                                                <v-btn
                                                    v-if="comment.content.length > 100"
                                                    text
                                                    @click="toggleText(postIndex, commentIndex)"
                                                >
                                                    {{ isExpanded[postIndex] && isExpanded[postIndex][commentIndex] ? 'Read less' : 'Read more' }}
                                                </v-btn>
                                            </p>
                                            <div class="d-flex align-center">
                                                <v-icon size="14" class="material-icons-round me-1 text-body cursor-pointer">
                                                    thumb_up
                                                </v-icon
                                                >
                                                <span class="text-sm me-2 text-body">3 likes</span>
                                                <v-icon size="14" class="material-icons-round me-1 text-body cursor-point">
                                                    share
                                                </v-icon>
...
export default {
  name: "Blog",
    ...
    methods: {
        ...
        toggleText(postIndex, commentIndex) {
            if (!this.isExpanded[postIndex]) {
                this.$set(this.isExpanded, postIndex, []);
            }
            this.$set(this.isExpanded[postIndex], commentIndex, !this.isExpanded[postIndex][commentIndex]);
        },
        initializeExpanded() {
            this.isExpanded = this.blog.map(post => post[0].comments.map(() => false));
        },
        async toggleLike(post) {
            BlogService.likePost(post[0].id)
                .then((response) => {
                    post['post'].isLikedByUser = !post['post'].isLikedByUser;
                    post['post'].likesCount = response.data.likesCount;
                })
                .catch(error => {
        ...
        async toggleLikeComment(comment) {
            BlogService.likeComment(comment)
                .then(() => {
