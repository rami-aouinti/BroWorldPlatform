    ...
    public function onActions(PageActionsEvent $event): void
    {
        ...
        if (!$event->isView('customer_details') && $canView) {
            $event->addAction('details', [
                ...
                'url' => $this->path('customer_details', [
                    'id' => $customer->getId(),
                ]),
            ]);
        }
        ...
        if ($this->isGranted('edit', $customer)) {
            $event->addEdit($this->path('admin_customer_edit', [
                'id' => $customer->getId(),
            ]), !$event->isView('edit'));
        }
        ...
        if ($this->isGranted('permissions', $customer)) {
            ...
            $event->addAction('permissions', [
                'title' => 'permissions',
                'translation_domain' => 'actions',
                'url' => $this->path('admin_customer_permissions', [
                    'id' => $customer->getId(),
                ]),
                'class' => $class,
            ]);
        ...
        if ($isListingView) {
            if ($customer->isVisible() && $this->isGranted('create_project')) {
                $event->addAction('create-project', [
                    'icon' => 'create',
                    'url' => $this->path('admin_project_create_with_customer', [
        ...
        if ($this->isGranted('view_project') || $this->isGranted('view_teamlead_project') || $this->isGranted('view_team_project')) {
            $event->addActionToSubmenu('filter', 'project', [
                'title' => 'project.filter',
                'translation_domain' => 'actions',
                'url' => $this->path('admin_project', [
        ...
        if ($event->isIndexView() && $this->isGranted('delete', $customer)) {
            $event->addDelete($this->path('admin_customer_delete', [
                'id' => $customer->getId(),
            ]));
        }

        if ($this->isGranted('report:customer') && $this->isGranted('report:other')) {
            $event->addActionToSubmenu('report', 'report_customer_monthly_projects', [
                'title' => 'report_customer_monthly_projects',
                'url' => $this->path('report_customer_monthly_projects', [
                    'customer' => $customer->getId(),
                ]),
        ...
        if ($this->isGranted('report:project') && $this->isGranted('budget_any', 'project')) {
            $event->addActionToSubmenu('report', 'daterange_projects', [
                'title' => 'report_project_daterange',
                'url' => $this->path('report_project_daterange', [
                    'customer' => $customer->getId(),
                ]),
                'translation_domain' => 'reporting',
            ]);
            $event->addActionToSubmenu('report', 'report_project_view', [
                'title' => 'report_project_view',
                'url' => $this->path('report_project_view', [
                    'customer' => $customer->getId(),
                ]),
                'translation_domain' => 'reporting',
            ]);
